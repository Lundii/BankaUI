...
let changePassword = () => {
  ...
  const url2 = `https://mighty-retreat-71326.herokuapp.com/api/v1/user/${user.data.id}/changePassword`;
  ...
  .then(function(data) {
    ...
    if (data.status === 200 ) {
      if (data.message){
        showErrMessModal('uRChpassword', 'Message', data.message);
      } 
    }
    else if (data.status === 401) {
      showErrMessModal('uRChpassword', 'Error', data.error);
      setTimeout(() => {
        window.location.href = '../../pages/signup.html'
      }, 2000)
    }
    else {
      let errorDiv = document.querySelector('#userpassErrors');
